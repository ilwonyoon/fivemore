...
- 손가락 개수로 타이머 길이를 정하고, 완료 후 같은 사진으로 한 번 더 이어갈 수 있다.
...
- Gentle Bell, Happy Bird, Tiny Drum, Star Pop 같은 완료 사운드 선택
...
카메라 화면에서 Vision의 손 인식으로 펼친 손가락 개수를 세고, 그 수를 분으로 사용한다.
두 손이면 합산한다. 다섯 손가락이면 5분, 열 손가락이면 10분이다.
...
- 같은 개수가 연속 프레임 동안 유지되어야 인식을 확정한다.
- 신뢰도가 낮은 관절은 접힌 것이 아니라 없는 것으로 처리한다.
- 촬영 진행 중에는 중복 촬영을 차단한다.

수용 기준:

- [ ] 손가락 개수와 시작된 타이머 길이가 일치한다.
...
따라서 브랜드 시스템 레퍼런스의 전체 화면 시스템 카메라 UI는 그대로 구현하지 않는다. 사진 프레임 안의 라이브 카메라 방식으로 번역한다. `Stats`와 사운드 선택 UI도 V1 기능 요구사항이 아니라 이후 탐색 자료다.
...
V1은 선택 화면 없이 하나의 짧고 부드러운 기본 완료 사운드만 제공한다. 브랜드 시스템 레퍼런스의 `Gentle Bell`을 1순위로 프로토타이핑한다.
