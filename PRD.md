...
V1은 `Home`과 `Memories` 두 개의 목적지만 제공한다. 두 번째 레퍼런스의 `Stats` 탭은 V1에서 제외하고,
항상 떠 있는 바텀 탭도 사용하지 않는다. 사진과 타이머가 화면의 중심이어야 하며, 상단 액션으로 충분히
빠르게 이동할 수 있다.

- Home 상단 우측에 `Memories` 아이콘을 둔다.
- Memories 상단 좌측에 `Home` 아이콘을 둔다.
- Settings는 Home 상단 우측의 두 번째 액션으로 연다.
- 활성 타이머 중에는 Memories/Home 전환 액션을 숨겨 핵심 상태가 사라지지 않게 한다.
